...
BUILD_W_MATRIX = True
INITIAL_CKPT   = './TEST27_150k_nobias_l1/lightning_logs/version_0/checkpoints/last.ckpt'
...
class ArgsT27(Args):
    ...
    n_facebook_samples   =   500_000  # 1_000_000 Facebook's training dataset
    ...
    GeM_p                = 3.0
    GeM_opt_p            = True
    checkpoint_base_path = f'./TEST27_{int(arc_classnum/1000)}k_nobias_l1'
...
class FacebookModel(pl.LightningModule):
    ...
    def to(self, device):
        if self.criterion_name.lower() == 'arcface_multigpu':
            ...
            print(" - Setting model device:", device)
            ret = super().to(device)
