...
assert len(KERNEL_GPUS) == 40

KERNEL_PROPS   = None # [0.8, 0.2]

QRY_AUGS_INTENSITY = 0.7
...
class ArgsT24_EffNetV2S(Args):
    ...
    backbone_name        = ['efficientnetv2_rw_s', 'efficientnetv2_rw_m', 'tf_efficientnetv2_l_in21ft1k',
                            ...
                            'nfnet_l0', 'eca_nfnet_l1', 'eca_nfnet_l2', 'eca_nfnet_l3'][0]
    ...
    n_imagenet_samples   =         0  # 1_431_167 ImageNet
    n_frm_face_samples   =         0  #   400_000  #  TotalSamples = 476_584 FrameFaces (Google's DeepFake dataset)
    
    arc_classnum         = n_facebook_samples + n_imagenet_samples + n_frm_face_samples
    arc_devices_v        = KERNEL_GPUS
    arc_w_prop_v         = KERNEL_PROPS
    arc_optimizer        = None
    ...
    GeM_p                = 3.0
    GeM_opt_p            = True
    checkpoint_base_path = f'./TEST24_{int(arc_classnum/1_000_000)}M_nobias'
...
class ArgsT25_NfNetl1_ImageNet(Args):
    OUTPUT_WH            = OUTPUT_WH
    DATASET_WH           = DATASET_WH
    BATCH_SIZE           = BATCH_SIZE
    N_WORKERS            = N_WORKERS
    ...
    n_facebook_samples   = 360_000  # 1_000_000 Facebook's training dataset
    n_imagenet_samples   = 40_000  # 1_431_167 ImageNet
    n_frm_face_samples   =      0  #   400_000  #  TotalSamples = 476_584 FrameFaces (Google's DeepFake dataset)
    ...
    GeM_p                = 3.0
    GeM_opt_p            = True
    checkpoint_base_path = f'./TEST25_{int(arc_classnum/1000)}k_nobias_l1'
    model_name           = 'arc_model'
...
if __name__ == '__main__':
    ...
    args = ArgsT25_EffNetV2S_ImageNet()
    print(args)
...
if __name__ == '__main__':
    _ = model.restore_checkpoint('./TEST25_200k_nobias_l1/lightning_logs/version_0/checkpoints/last.ckpt')
...
#     if 'criterion' in k:
#         print('normed:', k)
#         sd[k] = l2_norm(sd[k] + 0.10 * (torch.rand(sd[k].shape, device=sd[k].device)-0.5), axis=0, eps=1e-06)
...
# _ = model.restore_checkpoint('./TEST22_AF_SGD_RLR/weight_256d_w_trn.ckpt')
...
if __name__ == '__main__':
    ...
    if not os.path.exists(W_path):
        WR = calc_w_matrix(model, ds_trn, batch_size=64, num_workers=15, device='cuda:0', img_key='ref_img')
        W0 = calc_w_matrix(model, ds_trn, batch_size=64, num_workers=15, device='cuda:0', img_key='qry_img')
        W1 = calc_w_matrix(model, ds_trn, batch_size=64, num_workers=15, device='cuda:0', img_key='qry_img')
        W2 = calc_w_matrix(model, ds_trn, batch_size=64, num_workers=15, device='cuda:0', img_key='qry_img')
    ...
    W = load_obj( os.path.join(args.checkpoint_base_path, 'W.pickle') )
...
if __name__ == '__main__':
    if (NEW_LR is not None) and (last_ckpt is not None):
        input(f'Do you want to change the LR to {NEW_LR:0.02e}??')
        
        print(f'Manually setting LR = {NEW_LR:02e}')
        sd = torch.load(last_ckpt, map_location='cpu')
        
        for i_o, o in enumerate(sd['optimizer_states']):
            for i_pg, pg_d in enumerate( o['param_groups'] ):
                old_lr = pg_d['lr']
                print(f' - OPT{i_o:d}, ParamGroup_{i_pg:02d}: LR = {old_lr:0.02e}  >> {NEW_LR:0.02e}  ')
                pg_d['lr'] = NEW_LR
                
                
        sd['lr_schedulers'][0]['patience'] = args.sched_patience
        sd['lr_schedulers'][0]['factor']   = args.sched_factor
        sd['lr_schedulers'][0]['_last_lr'] = [NEW_LR for i in sd['lr_schedulers'][0]['_last_lr']]
        
        
        last_ckpt = last_ckpt.replace('.ckpt', 'new_lr.ckpt')
        torch.save(
            sd,
            last_ckpt,
            )
...
# trainer.save_checkpoint(os.path.join(args.checkpoint_base_path, 'after_fit.ckpt'))
