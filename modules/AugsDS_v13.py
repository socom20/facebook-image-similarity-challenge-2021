...
if __name__ == '__main__':
    import torch
    seed = 3134
    print(f' - AugsDS, Setting SEED = {seed}')
    random.seed(seed)
...
def get_path_from_sample_id(sample_id, ds_dir='./dataset', ext='jpg'):
    if sample_id[0] == 'Q':
        ...
        image_path = os.path.join(ds_dir, 'face_frames', f'{sample_id}.{ext}')
    else:
        raise Exception('???')
...
#         img = img.resize(output_wh)
    ...
    OVERLAY_IMAGE = False
    OVERLAY_ONTO_SCREENSHOT  = False
    
    if random.random() <= 0.10:
        img = imaugs.scale(
            image=img,
            factor=0.6 + 0.40 * random.random(),
            interpolation=None,
            metadata=metadata_v
        )

    if random.random() <= 0.10 * I:
        img = imaugs.rotate(
            image=img, 
            degrees=I * random.randint(0, 360),
            metadata=metadata_v
        )
    ...
    if random.random() <= 0.10:
        img = imaugs.encoding_quality(
            image=img, 
            quality=int( I * random.randint(10, 50) ),
            metadata=metadata_v
        )
    ...
    if random.random() <= 0.20:
        img = imaugs.color_jitter(
            image=img,
            brightness_factor=N + I*( 0.25 + 1.5 * random.random() ),
            contrast_factor=N + I*( 0.25 + 1.5 * random.random() ),
            saturation_factor=N + I*( 0.2 + 50*np.exp( 5*random.random() )/300.0 ),
            metadata=metadata_v,
        )
    ...
    if CHECK_IMGSIZE and (min(img.size) < 0.5 * min(output_wh)):
        # Preventing errors
#         print('Rescaling 1')
        img = resize(
            img,
            ...
            height=output_wh[1],
            metadata=metadata_v,
            resample=random.choice([0, 1, 2, None]),
        )

    if not PERSPECTIVE_TRANSFORM and (random.random() <= 0.10*I):
        img = imaugs.crop(
            image=img,
    ...
    if not PERSPECTIVE_TRANSFORM and (random.random() <= 0.05 * I):
        W, H = img.size
        if random.random() <= 0.5 or W == H:
            img = imaugs.pad(
                image=img,
                w_factor=0.10*random.random() * I,
                h_factor=0.10*random.random() * I,
    ...
    if not PERSPECTIVE_TRANSFORM and (random.random() <= 0.10 * I):
        if overlay_img_ids_v is None:
            neg_img = read_image(f'T{random.randint(0,999_999):06d}', ds_dir=ds_dir)
        ...
        if random.random() <= 0.50:
            ...
            overlay_size = 0.40 + 0.40*random.random()
            img = imaugs.overlay_image(
                image=neg_img,
                overlay=img,
    ...
    if random.random() <= 0.10 * I:
        if random.random() <= 0.50:
            img = imaugs.blur(
                img,
                radius=I * (0.1 + random.randint(1,4)),
                metadata=metadata_v
            )
...
#         print('Rescaling 2')
    ...
    if not OVERLAY_IMAGE and (random.random() <= 0.10 * I):
    ...
    if  not OVERLAY_IMAGE and not PERSPECTIVE_TRANSFORM and (random.random() <= 0.05 * I):
...
#         )
    ...
    if not GRAY_SCALE and not OVERLAY_ONTO_SCREENSHOT:
        if (random.random() <= 0.05 * I):
        ...
        if (random.random() <= 0.05 * I):
            n = max( 1, int(0.05* min(img.shape[:2])) )

            img = CA.shift_channels(
                img,
                shift_v=[random.randint(-n,n), random.randint(-n,n)],
                metadata=metadata_v
            )
...
class FacebookSyntheticDataset(Dataset):
    def __init__(
        ...
        self.qry_aug_intensity = qry_aug_intensity
        ...
        while not aug_ok:
            try:
                ...
                qry_img, qry_metadata_v = query_augmentation(
                    ...
                    intensity=self.qry_aug_intensity,
                )
...
if __name__ == '__main__':
    n_facebook_samples = 1_000_000
    n_imagenet_samples = 1_000_000
    n_frm_face_samples = 200_000
    
    
    trn_samples_id_v = np.array(
        [f'T{i:06d}' for i in range(0, n_facebook_samples)] +
        [f'I{i:07d}' for i in range(0, n_imagenet_samples)] +
        [f'F{i:06d}' for i in range(0, n_frm_face_samples)] 
    )
    ...
    ds_trn = FacebookSyntheticDataset(
        ...
        norm_type='imagenet',
        qry_aug_intensity=QRY_AUGS_INTENSITY,
        verbose=True,
    )
    
    for i in range(50):
        ds_trn.plot_sample(9585)
        plt.show()
...
# if __name__ == '__main__':
#     for i, data in enumerate( tqdm(DataLoader(ds_trn, batch_size=128, num_workers=12, timeout=0, shuffle=True)) ):
#         pass
...
# img_org = read_image('Q02021', ds_dir=DS_DIR)
# s_v = []
...
# meta_v = []
# while all( [m['name'] != 'resize' for m in  meta_v]):
#     img, meta_v = query_augmentation(
#         img_org.copy(),
#         OUTPUT_WH,
#         qry_wh_info_d=None,
#         ds_dir=DS_DIR,
