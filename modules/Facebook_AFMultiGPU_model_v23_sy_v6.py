...
BATCH_SIZE   = 64 #128#256#384 #132 # 64 # 22
N_WORKERS    = 10  #8 #24
...
N_FACEBOOK = 1_000_000
N_IMAGENET =   400_000
N_FRM_FACE =   100_000

BACKBONE_NAME = ['efficientnetv2_rw_s', 'efficientnetv2_rw_m', 'tf_efficientnetv2_l_in21ft1k'][1]
...
class ArgsT23_EffNetV2(Args):
    ...
    GeM_p                = 3.0
    GeM_opt_p            = True
    checkpoint_base_path = './TEST23_AF_1.5M_CFG_A_oct8'
...
def calc_w_matrix(
    ...
    with torch.no_grad():
        for data in tqdm(dl):
            y = model.forward(data['ref_img'].to(device))        
            w_v.append(
                y.detach().cpu().numpy()
            )
...
# W = calc_w_matrix(model, ds_trn, batch_size=64, num_workers=15, device='cuda:0')
# save_obj(os.path.join(args.checkpoint_base_path, 'W.pickle'))
